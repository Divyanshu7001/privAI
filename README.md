...
`privAI` is a browser-extension-based privacy assistant that monitors what a user is about to post on supported social platforms (LinkedIn, Facebook, Instagram) and runs client-side checks (with local ML assistance) to flag sensitive information before it is published.
...
- **`components/content/`**:
  ...
  - [monitoring-helpers.js](file:///d:/Projects/privAI/extension/components/content/monitoring-helpers.js): Intercepts user click actions on buttons labeled "Post", "Share", "Reply", or "Send".
...
_Load the output extension files under the `.output` directory into your browser's Developer Extensions panel._
...
_Note: Faster-Whisper is configured by default for CUDA execution (`device="cuda"`). Update `main_copy.py` (line 20) to `device="cpu"` if running on non-GPU instances._
